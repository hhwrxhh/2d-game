...
void BrickWall::renderBrick(const EBrickLocation eBrickLocation) const
{
	...
	{
		m_sprites[static_cast<size_t>(state)]->render(m_position + m_blockOffesets[static_cast<size_t>(eBrickLocation)], m_size / 2.f, m_rotation, m_layer);
	}
}
...
void BrickWall::update(double delta)
{
}
